...
$( document ).ready(function() {
  convertCSVToJSON('https://cs374.s3.ap-northeast-2.amazonaws.com/country_capital_pairs.csv', function(data){
  window.pairs = data;
  ...
  answer.value = '';
  var object = country_capital_pairs[getRandomInt(0,country_capital_pairs.length -1)];
  ...
  // Real-time listener
  db.collection('capitals').orderBy('date').onSnapshot(snapshot=>{
...
// Undo feature
document.getElementById("pr3__undo").addEventListener('click', function (){


	db.collection("history").orderBy('timestamp', 'desc').limit(1).get()
	.then(res => {
		res.forEach(element => {
		  ...
		  if (status == 'removed'){
		  	...
		  	db.collection('capitals').orderBy('date', 'desc').limit(1).get().then(res2 => {
...
// Clear all
document.getElementById('pr3__clear').addEventListener('click',function () {
	countryL = new Array;
	capitalL = new Array;
	givenCapitalL = new Array;
	db.collection("capitals").get()
	.then(res => {
		res.forEach(element => {
...
// Restart Everything
document.getElementById('pr3__restart').addEventListener('click',function (){
...
function convertCSVToJSON(strUlr, callback) {
    fetch(strUlr).then(response => response.text())
...
function submit(){
	...
	var date = Date.now();
	if (capital.toLowerCase() != document.getElementById('pr2__answer').value.toLowerCase()){
		db.collection('capitals').add({
			country: country,
			capital: capital,
...
function insert(doc) {
	...
	if (doc.data().givenCapital == doc.data().capital){
		$('#parent > tbody > tr').eq(row-1).after("<tr class='element' id='"+doc.id+"'>  <td class='correct' onmouseover='changeMap(this)' id='country'>"+doc.data().country+"</td> <td class='correct' id='capital1'>"+doc.data().capital+" </td><td class='correct' onmouseover='changeMapCapital(this)'>"+doc.data().capital+"<button onClick='deleteEntry(this)'>Delete</button></td></tr>");
	} else{
		$('#parent > tbody > tr').eq(row-1).after("<tr class='element' id='"+doc.id+"'>  <td class='incorrect' onmouseover='changeMap(this)' id='country'>"+doc.data().country+"</td> <td class='incorrect' ><del id='capital2'>"+ doc.data().givenCapital +"</del></td><td class='incorrect' id='country2' onmouseover='changeMapCapital(this)'>"+doc.data().capital+"<button onClick='deleteEntry(this)'>Delete</button></td></tr>");
	}
}
...
function deleteEntry(elem){
	...
	db.collection('capitals').doc(id).get().then(function(doc){
		db.collection('history').add({
...
var changeMapCapital = function (elem) {
    ...
    elem.onmouseover = function() {
    	if ( $iframe.length ) {
	        ...
	        $iframe.attr('style',"color: black; filter: grayscale(100%); transition: transform .2s;");

	        elem.parentNode.style="background-color: lightgray"  
	    }
        // Set timeout to be a timer which will invoke callback after 1s
        timeout = setTimeout(function(){
        	$iframe.attr('src',"https://www.google.com/maps/embed/v1/place?key=AIzaSyAOR25-vse-iJ0pD5RrnhOqCOhHd2A1wJU&q="+elem.innerHTML.split('<')[0]+"&language=en&zoom=4");
        }, 500);
...
function loadIframe(country) {
    var $iframe = $('#ifrm');
    if ( $iframe.length ) {
        $iframe.attr('src',"https://www.google.com/maps/embed/v1/place?key=AIzaSyAOR25-vse-iJ0pD5RrnhOqCOhHd2A1wJU&q="+country+"&language=en");   
        return false;
    }
    return true;
}
...
function filterCheck(){
    ...
    $("tr.element").find("td").each(function() {
      $this = $(this);
